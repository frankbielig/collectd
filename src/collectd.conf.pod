...
=head2 Plugin C<write_dlt>

The C<write_dlt> plugin send metrics as INFO messages to Diagnostic Log 
and Trace daemon.
...
Customization of application identifier used for tagging DLT messages. 
By default the value is set to "CLTD". Because of the DLG standard, 
please note that only the first four characters are used for the ID. 
...
Matches the actual value of data sources against the previous state.

Available options:

=over 4

=item B<DiffAbs> I<Value>

Sets the maximal difference between the previous value and the current one.

=item B<DiffRel> I<Value>

Sets the maximal relative difference determined as the quotient of the 
absolute difference and the previous value
